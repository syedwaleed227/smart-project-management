...
- **Expenses** — list, submit; amounts over the threshold auto-open an
  approval record.
- **Approval Center** — pending queue with approve/reject + reason, backed by
  the database **workflow engine** (multi-step routing, authorization, entity
  outcome, history) via `decide_approval_step`.
- **Notifications** — in-app feed with unread badge in the top bar and
  mark-read / mark-all-read. Generated by DB triggers + the workflow engine
  (task assigned, approval pending/accepted/rejected, budget exceeded).
